...
func TestParseArgsTrueSkill(t *testing.T) {
	config, err := parseArgs([]string{"trueskill", "--start-date", "2026-01-01", "--end-date", "2026-06-30", "--min-tournaments", "3"})
	if err != nil {
		t.Fatal(err)
	}
	if config.system != "trueskill" || config.minimumTournaments != 3 {
		t.Fatalf("unexpected config: %#v", config)
	}
}
...
func TestParseArgsRequiresFlags(t *testing.T) {
	...
	if _, err := parseArgs([]string{"glicko", "--start-date", "2026-01-01", "--end-date", "2026-06-30", "--min-tournaments", "3"}); err == nil {

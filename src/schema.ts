...
export function applySchema(db: Database): void {
  ...
  if (!hasColumn(db, "tournaments", "tournament_date")) {
    db.exec(`ALTER TABLE tournaments ADD COLUMN tournament_date TEXT;`);
  }
  if (!hasColumn(db, "players", "braacket_league_player_id")) {
    db.exec(`ALTER TABLE players ADD COLUMN braacket_league_player_id TEXT;`);
  }
  if (!hasColumn(db, "tournament_players", "braacket_league_player_id")) {
    db.exec(`ALTER TABLE tournament_players ADD COLUMN braacket_league_player_id TEXT;`);
  }
  if (!hasColumn(db, "matches", "player1_tournament_player_id")) {
    db.exec(`ALTER TABLE matches ADD COLUMN player1_tournament_player_id INTEGER;`);
  }
  if (!hasColumn(db, "matches", "player2_tournament_player_id")) {
    db.exec(`ALTER TABLE matches ADD COLUMN player2_tournament_player_id INTEGER;`);
  }
  if (!hasColumn(db, "matches", "winner_tournament_player_id")) {
    db.exec(`ALTER TABLE matches ADD COLUMN winner_tournament_player_id INTEGER;`);
  }
  db.exec(`
